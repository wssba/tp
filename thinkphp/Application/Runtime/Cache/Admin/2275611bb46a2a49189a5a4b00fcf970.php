...
margin-left:-280px;position:absolute;width:560px; overflow:visible;
    ...
    <body>
        ...
        <div class="login">
<!--            设置验证码  onclick事件是为了点击验证码在生成一个新的验证码（看不清换一张）-->
            <img src="/admin/admin/createimg" id="verity_code" onclick="this.src='/admin/admin/createimg/'+Math.random()"/>
        ...
        <script>
            ...
            $("#verity_code").attr("src",'/admin/admin/createimg?random='+Math.random());         
             ...
             $.post('/admin/admin/login',$(".loginform").serialize(),function(data){  
        if(data.status=="成功"){
                 window.location.href="http://www.etp.com/admin/admin/index";
             }if(data.status=="密码不对"){
                 alert('密码不正确');
                 return false;
